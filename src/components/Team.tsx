...
export default function Team() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Desktop: Full clip-path reveal + scale + hover effects
      mm.add('(min-width: 1024px)', () => {
        cards.forEach((card) => {
          ...
          if (image) gsap.set(image, { clipPath: 'circle(0% at 50% 50%)', scale: 1.2 });
        ...
        cards.forEach((card, index) => {
          ...
          cardsTl.to(card, { opacity: 1, y: 0, scale: 1, duration: 0.6, ease: 'power3.out' }, staggerOffset);
          if (image) cardsTl.to(image, { clipPath: 'circle(75% at 50% 50%)', scale: 1, duration: 0.8, ease: 'power3.out' }, staggerOffset + 0.1);
          if (content) cardsTl.to(content, { y: 0, opacity: 1, duration: 0.5, ease: 'power3.out' }, staggerOffset + 0.3);
        });
        ...
        cards.forEach((card) => {
          ...
          card.addEventListener('mouseenter', () => {
            gsap.to(card, { scale: 1.03, boxShadow: '0 0 40px hsl(199 89% 60% / 0.2)', duration: 0.3 });
            if (techOverlay) gsap.to(techOverlay, { opacity: 1, duration: 0.25 });
          });

          card.addEventListener('mouseleave', () => {
            gsap.to(card, { scale: 1, boxShadow: '0 4px 30px rgba(0, 0, 0, 0.3)', duration: 0.3 });
            if (techOverlay) gsap.to(techOverlay, { opacity: 0, duration: 0.25 });
          });
        });
      });

      // Mobile/Tablet: Simple fade-up reveal, no clip-path
      mm.add('(max-width: 1023px)', () => {
        ...
        cards.forEach((card) => {
          ...
          const image = card.querySelector('.team-image');
          const content = card.querySelector('.team-content');
          if (image) gsap.set(image, { clipPath: 'circle(75% at 50% 50%)', scale: 1 });
