...
export default function HowItWorks() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Desktop: Sticky scroll with SVG progress
      mm.add('(min-width: 1024px)', () => {
        ...
        ScrollTrigger.create({
          ...
          onUpdate: (self) => {
            ...
            const currentStep = Math.min(Math.floor(progress * cards.length), cards.length - 1);
            setActiveStep(currentStep);

            if (progressLine) {
              const lineLength = progressLine.getTotalLength();
              gsap.set(progressLine, { strokeDashoffset: lineLength * (1 - progress) });
            }
            ...
            cards.forEach((card, index) => {
              ...
              if (isActive) gsap.set(card, { scale: 1, opacity: 1, y: 0 });
              else if (isPast) gsap.set(card, { scale: 0.92, opacity: 0.5, y: -20 });
              else gsap.set(card, { scale: 0.85, opacity: 0.3, y: 30 });
            });
          },
        });
      });
      ...
      mm.add('(min-width: 1024px)', () => {
        cards.forEach((card) => {
          ...
          card.addEventListener('mouseenter', () => {
            gsap.to(card, { y: -5, boxShadow: '0 0 40px hsl(199 89% 60% / 0.25)', duration: 0.25 });
          });

          card.addEventListener('mouseleave', () => {
            gsap.to(card, { y: 0, boxShadow: '0 4px 30px rgba(0, 0, 0, 0.3)', duration: 0.25 });
          });
        });
      });
    }, section);
