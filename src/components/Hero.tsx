 ...
 * - Clean split-text animation
 * - RAF-throttled parallax (desktop only)
 */
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Master timeline - works on all devices
      const masterTl = gsap.timeline({
        delay: 0.3,
        defaults: { ease: 'power3.out' },
      });

      masterTl.fromTo(tagline, { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6 });
      ...
      masterTl.fromTo(description, { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8 }, '-=0.6');

      masterTl.fromTo(cta.children, { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, stagger: 0.12 }, '-=0.5');
      gsap.set(cta, { opacity: 1 });

      const statCards = stats.querySelectorAll('.stat-card');
      gsap.set(stats, { opacity: 1 });
      masterTl.fromTo(statCards, { y: 40, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, stagger: 0.1 }, '-=0.3');

      masterTl.fromTo(scrollIndicator, { opacity: 0, y: 10 }, { opacity: 1, y: 0, duration: 0.4 }, '-=0.2');

      // Desktop-only animations with matchMedia
      ...
      // Scroll animations - balanced for all devices
      mm.add('(min-width: 769px)', () => {
